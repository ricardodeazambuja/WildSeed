...
> **Now first-class CLI.** The generators and benchmarks below have `wildseed`
> equivalents (marked **CLI:** in the table); the whole recipe is one command,
> `wildseed scenario --profile vio_lio --seed N`. The `tools/*.py` scripts are kept
> (thin shims over `src/wildseed/core/`) so these reproduction commands keep working
> and stay runnable directly. User guide: **`docs/VIO_LIO_FEATURES.md`**.

| Script | What it does |
|--------|--------------|
| **`vio_bench.py`** | **CLI: `wildseed benchmark vio`.** **The VIO benchmark.** Measures descriptor **data-association quality under motion** — the metric that predicts VIO failure (perceptual **aliasing**: repeated/indistinct features that break matching), which feature *count* misses. Renders a canonical translation+yaw trajectory over the current `models/` world and reports Lowe-ratio rejection, essential-matrix **inlier ratio**, inliers/pair, self-ambiguity + a GOOD/ALIASING-RISK verdict. `python3 tools/vio_bench.py --tag myworld` (or `--ground-modes patchy,uniform_t1` to A/B). |
...
| **`rtf_bench.py`** | **CLI: `wildseed benchmark rtf`.** **RTF-under-load harness** — the COST gauge. Launches a real `gz sim -s -r` server on a rig world, attaches cam/lidar consumers, waits for the sim clock to advance (skips load stall), and samples `real_time_factor` off `/stats`. Reports `window_rtf` / `rtf_min` / `load_wait_s` / `stalled`. Keep the operating point where `rtf_min` ≥ ~0.5. |
| **`lidar_spread.py`** | **CLI: `wildseed benchmark lidar`.** **LIO axis (V3)** the camera benchmark can't see: gpu_lidar range **roughness**. `ring_roughness_m` = mean std of Δrange between adjacent azimuth beams (~0 over flat ground, rises with clutter/relief), plus `range_std_m` / `near_frac` / `finite_frac`. |
| `corridor_map.py` | **CLI: `wildseed corridor-map`** (logic in `core/density_maps.py`). **Steered-scatter (c) plumbing**: paints a driving-corridor density map (white band at the drive line, `--soft` Gaussian taper) for `generate --density-maps` → the object budget lands where the vehicle drives (high local density, low total count). Baked into `scenario --profile vio_lio`. |
| `heightmap_relief.py` | **CLI: `wildseed heightmap`** (logic in `core/heightmap.py`). **Geometric-relief (d2) plumbing**: writes a hi-res gz `<heightmap>` ground with cm–dm surface roughness on a FLAT drivable macro (Ogre2 Terra: GPU-tessellated + LOD'd, one static mesh) + injects the rig. Carries relief the Nyquist-limited WildSeed mesh (d1) can't, at RTF 1.0. Measure with `benchmark rtf` / `benchmark lidar` / `vio_bench --heightmap`. |
| **`vio_validate.py`** | **CLI: `wildseed benchmark validate`.** **End-to-end VIO/LIO ATE validation (Phase C)** — turns the proxies into real trajectory drift. Self-contained reference estimator (no ROS): monocular ORB+essential-matrix VO (GT-scaled) + point-to-point ICP LIO, Umeyama-aligned to TUM ground truth from `record --dataset`. Reports segment/relative drift, global ATE, essential-matrix inlier fraction, and tracking-failure counts per run. `--mask-label 8` excludes moving (distractor) pixels via the recorded segmentation stream — the dynamics-gate comparison; `--calib models/sensor_rig/rig_calibration.json` scores with the TRUE camera intrinsics instead of the nominals — the instrument-gate comparison. `python3 tools/vio_validate.py runs/recipe runs/baseline`. |
